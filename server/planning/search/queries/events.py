...
def search_calendars(params: Dict[str, Any], query: elastic.ElasticQuery):
    ...
    if num_calendars == 1:
        query.must.append(elastic.term(field="calendars.qcode", value=calendars[0]))
    elif num_calendars > 1:
        query.must.append(elastic.terms(field="calendars.qcode", values=calendars))


def search_no_calendar_assigned(params: Dict[str, Any], query: elastic.ElasticQuery):
    if strtobool(params.get("no_calendar_assigned", False)):
        query.must_not.append(elastic.field_exists("calendars"))
...
def search_date_today(params: Dict[str, Any], query: elastic.ElasticQuery):
    if params.get("date_filter") == elastic.DATE_RANGE.TODAY:
        ...
        query.filter.append(
            elastic.bool_or(
                [
                    elastic.range_today(elastic.ElasticRangeParams(field="dates.start", time_zone=time_zone)),
                    elastic.range_today(elastic.ElasticRangeParams(field="dates.end", time_zone=time_zone)),
                    elastic.bool_and(
                        [
                            elastic.field_range(
...
def search_date_tomorrow(params: Dict[str, Any], query: elastic.ElasticQuery):
    if params.get("date_filter") == elastic.DATE_RANGE.TOMORROW:
        ...
        query.filter.append(
            elastic.bool_or(
                [
                    elastic.range_tomorrow(elastic.ElasticRangeParams(field="dates.start", time_zone=time_zone)),
                    elastic.range_tomorrow(elastic.ElasticRangeParams(field="dates.end", time_zone=time_zone)),
                    elastic.bool_and(
                        [
                            elastic.field_range(
                                elastic.ElasticRangeParams(
...
def search_date_last_24_hours(params: Dict[str, Any], query: elastic.ElasticQuery):
    if params.get("date_filter") == elastic.DATE_RANGE.LAST_24:
        ...
        query.filter.append(
            elastic.bool_or(
                [
                    elastic.range_last_24_hours(elastic.ElasticRangeParams(field="dates.start", time_zone=time_zone)),
                    elastic.range_last_24_hours(elastic.ElasticRangeParams(field="dates.end", time_zone=time_zone)),
                    elastic.bool_and(
                        [
                            elastic.field_range(
                                elastic.ElasticRangeParams(
                            ...
                            elastic.field_range(
                                elastic.ElasticRangeParams(field="dates.end", gt="now", time_zone=time_zone)
                            ),
                        ]
                    ),
                ]
