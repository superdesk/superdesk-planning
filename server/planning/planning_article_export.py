...
def generate_text_item(items, template_name, resource_type):
    ...
    for item in items:
        ...
        users = get_resource_service("users").find(where={"_id": {"$in": users}})

        desks = get_resource_service("desks").find(where={"_id": {"$in": desks}})

        for u in users:
            name = u.get(
                "display_name",
                "{0} {1}".format(u.get("first_name"), u.get("last_name")),
            )
