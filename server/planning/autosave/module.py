...
async def _on_assignment_updated(updates: dict, original: dict) -> None:
    ...
    if "assigned_to" not in updates and "priority" not in updates:
        ...
        logger.debug(
            "planning:autosave_assignment_skipped assignment_id=%s reason=no_relevant_fields",
            original.get("_id"),
        )
        return
    ...
    if current_request and "/planning" in current_request.path:
        ...
        logger.debug(
            "planning:autosave_assignment_skipped assignment_id=%s reason=planning_request_path path=%s",
            original.get("_id"),
    ...
    logger.info(
        "planning:autosave_assignment assignment_id=%s planning_item=%s coverage_item=%s updated_fields=%s",
        assignment.get("_id"),
