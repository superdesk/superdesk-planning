...
from planning.types import UnifiedPlanningResource, AssignmentResourceModel
...
class DataUpdate(BaseDataUpdate):
    ...
    async def forwards(self, _collection: AsyncIOMotorCollection, _database: AsyncIOMotorDatabase) -> None:
        await self._fix_unified_planning()
        await self._fix_assignments()

    async def _fix_unified_planning(self) -> None:
    ...
    async def iterate_items(self, service: AsyncResourceService) -> AsyncGenerator[dict, None]:
        ...
        query: dict
        if service.resource_name == "assignments":
            query = {"query": {"bool": {"must": _get_query("planning.")}}}
        elif service.resource_name == "unified_planning":
            query = {
                "query": {
                    "bool": {
                        "should": [
