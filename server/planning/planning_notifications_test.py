...
class NotificationTests(TestCase):
            ...
            [
                {
                    ...
                    "user_preferences": {"slack:notification": {"enabled": True}},
                }
            ],
        )
    ...
    def test_desk_notification(self, sc):
        try:
            PlanningNotifications()._notify_slack(
                "hdskjgdsjg",
                target_user=None,
                target_desk=self.desk_ids[0],
                target_desk2=None,
                message="hello world by Unknown",
            )
        except Exception:
    ...
    def test_user_notification(self, sc):
        try:
            PlanningNotifications()._notify_slack(
                "djkjhkjhdkjhdk",
                target_user=self.user_ids[0],
                target_desk=None,
                target_desk2=None,
                message="hello user from world by Unknown",
            )
        except Exception:
            self.assertTrue(False)
