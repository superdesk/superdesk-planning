...
class PlanningService(superdesk.Service):
        ...
        for coverage in updates.get('coverages') or []:
            ...
            for i, scheduled_update in enumerate(schedule_updates):
                ...
                if (coverage_schedule and scheduled_update_schedule <= coverage_schedule):
                    raise SuperdeskApiError(message="Scheduled updates of a coverage must be after the original coverage.")
    ...
    def remove_coverage_entity(self, coverage_entity, original_planning, entity_type='coverage'):
        ...
        if assignment and assignment.get('state') not in [WORKFLOW_STATE.DRAFT, WORKFLOW_STATE.CANCELLED]:
            raise SuperdeskApiError.badRequestError('Assignment already exists. {} cannot be deleted.'.format(entity_type.capitalize()))

        updated_coverage_entity = deepcopy(coverage_entity)
        assigned_to = updated_coverage_entity.pop('assigned_to', None)
        if assigned_to:
            if original_planning.get('state') == WORKFLOW_STATE.CANCELLED or coverage_entity.get('workflow_status') \
                    not in [WORKFLOW_STATE.CANCELLED, WORKFLOW_STATE.DRAFT]:
                raise SuperdeskApiError.badRequestError(
                    '{} not in correct state to remove assignment.'.format(entity_type.capitalize()))
            # Removing assignment
            get_resource_service('assignments').delete(lookup={'_id': assigned_to.get('assignment_id')})
            assignment = {
                'planning_item': original_planning.get(config.ID_FIELD),
                'coverage_item': coverage_entity.get('coverage_id')
            }
            if entity_type == 'scheduled_update':
                assignment['scheduled_update_id'] = coverage_entity.get('scheduled_update_id')
    ...
    def update_scheduled_updates(self, updates, original, coverage, original_coverage):
        for s in (coverage.get('scheduled_updates') or []):
            original_scheduled_update = next((original_s for original_s in (original_coverage.get('scheduled_updates') or [])
                                      if s['scheduled_update_id'] == original_s.get('scheduled_update_id')), None)

            if original_scheduled_update and original_scheduled_update.get('workflow_status') == WORKFLOW_STATE.DRAFT and \
