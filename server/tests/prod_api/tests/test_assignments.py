...
def test_service_get(prodapi_app_with_data):
    """Test fetching items using `assignments` service

    :param prodapi_app_with_data: prod api app with filled data
    """

    with prodapi_app_with_data.app_context():
        # TODO-ASYNC[AssignmentsService] - Convert this to async when test is updated to async
        items_service = get_resource_service("assignments")

        assert len(list(items_service.get(req=None, lookup={}))) == 4


def test_readonly(prodapi_app_with_data, prodapi_app_with_data_client):
    """Ensure that `assignments` endpoint is readonly
    ...
    with prodapi_app_with_data.test_request_context():
        for method, status in (
            ...
            # we send a request
            resp = getattr(prodapi_app_with_data_client, method)(url_for("assignments|resource"))
            # we get a response
...
def test_excluded_fields(prodapi_app_with_data, prodapi_app_with_data_client):
    ...
    with prodapi_app_with_data.test_request_context():
        # list
        resp = prodapi_app_with_data_client.get(url_for("assignments|resource"))
        ...
        resp = prodapi_app_with_data_client.get(
            url_for("assignments|item_lookup", _id=item["_id"]),
        )
        resp_data = json.loads(resp.data.decode("utf-8"))
